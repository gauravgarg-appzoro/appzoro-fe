...
import { Button, Form, Row, Col, Image, Collapse, Modal } from 'react-bootstrap';
...
const IndustryCreate = () => {
    ...
    const isNew = true; // Always create mode for this file
    ...
    const handleSubmit = async (e, isDraft = false) => {
        ...
        try {
            ...
            const result = await adminService.createIndustry({ ...formData, slug, published_at });
    ...
    return (
        <AdminLayout title="Create Industry Page">
            <AdminFormPageHeader
                                ...
                                {formData.industryFeatures.map((item, idx) => (
                                    <div key={idx} className="mb-3 bg-white border rounded">
                                        ...
                                        <Collapse in={!collapsedItems[`features-${idx}`]}>
                                            <div className="p-3">
                                                <FormField label="Feature Title" value={item.featuresTitle} onChange={(e) => updateFeature(idx, 'featuresTitle', e.target.value)} name="" />
                                                <div>
