...
const BlogArchives = () => {
    ...
    const fetchArchives = async () => {
        ...
        try {
            const [dataResult, countResult] = await Promise.all([
                adminService.getArchives({ page, limit, sort }),
                adminService.getArchivesCount()
            ]);

            if (dataResult.success) {
                setArchives(dataResult.data);
            } else {
    ...
    const handleDelete = async () => {
        ...
        const result = await adminService.deleteArchive(deleteId);
        if (result.success) {
    ...
    return (
        <AdminLayout title="Archives">
            ...
            <div className="d-flex justify-content-between align-items-start mb-4">
                <div>
                    <h2 className="fw-bold mb-1">Archives</h2>
                    <p className="text-secondary mb-0 small">{totalCount} entries found</p>
                </div>
                <Link href="/admin/blogs/archives/create" passHref>
                    <Button variant="primary" className="d-flex align-items-center px-4">
                        <FaPlus className="me-2" size={12} /> Add New Archive
                    </Button>
                </Link>
            </div>
                ...
                <Table hover className="mb-0 align-middle">
                    <thead className="bg-light">
                        <tr>
                            ...
                            <th className="py-3 border-0 fw-bold">Published Date</th>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan="5" className="text-center py-4 text-muted">No archives found</td>
                            </tr>
                        ) : (
                            archives.map((archive) => (
                                <tr key={archive.id || archive._id} className="border-bottom">
                                    <td className="ps-3"><Form.Check type="checkbox" /></td>
                                    <td className="fw-medium text-dark">{archive.name}</td>
                                    <td className="text-secondary small">{archive.slug}</td>
                                    <td className="text-secondary small">
                                        {archive.published_at ? new Date(archive.published_at).toLocaleDateString() : '-'}
                                    </td>
                                    <td className="text-end pe-3">
                                        <Link href={`/admin/blogs/archives/${archive.id || archive._id}`} passHref>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => confirmDelete(archive.id || archive._id)}
                                        >
                                            <FaTrash size={14} />
                                        </Button>
                                    </td>
            ...
            <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
                ...
                <Modal.Body>Are you sure you want to delete this archive? This action cannot be undone.</Modal.Body>
                <Modal.Footer>
