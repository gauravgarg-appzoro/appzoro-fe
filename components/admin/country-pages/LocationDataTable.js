...
const LocationDataTable = ({ locations = [], onDelete }) => {
            ...
            <Table hover className="mb-0 align-middle">
                <thead className="bg-light">
                    <tr>
                        ...
                        <th className="border-0 py-3 fw-bold">Location_title <FaSort size={10} className="ms-1 text-secondary" /></th>
                        <th className="border-0 py-3 fw-bold">Slug</th>
                        <th className="border-0 py-3 fw-bold">Section2_title</th>
                ...
                <tbody>
                    {locations.length === 0 ? (
                        <tr><td colSpan="7" className="text-center text-muted py-4">No location data found</td></tr>
                    ) : locations.map((item) => (
                        <tr key={item._id || item.id} className="border-bottom">
                            ...
                            <td className="fw-medium text-dark">{item.location_title || '—'}</td>
                            <td className="text-secondary small">{item.slug || '—'}</td>
                            <td className="text-secondary">{item.section2_title || '—'}</td>
                            <td><StatusBadge status={getState(item)} /></td>
                            <td className="text-end pe-3">
                                <Link href={`/admin/country-pages/location-data/${item._id || item.id}`} passHref>
